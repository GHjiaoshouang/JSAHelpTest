...
`为了防止代码滥用，本类库设有专属的签名验证机制，代码运行需要在已被签名的机器上。后期会不断向里面填充各类实用帮助类库。签名器下载地址请致邮：[1924038917jsa@gmail.com]或关注GitHub:[https://github.com/GHjiaoshouang]。如有侵权请联系我。
`谢谢。
```
...
```c#
JSAHelpEN.toolsFactory.Logger.InitializationSetting.logPath = JSAHelpEN.toolsFactory.LocalFileManage.FilePathEnumerate.ProgramDebug;
```
